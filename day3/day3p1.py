...
def check_around(list, x, y):
    ...
    for i in range(-1, 2):
        for j in range(-1, 2):
            ...
            if (list[special_idx(x + i, list_len)][special_idx(y + j, list_len)] in special_characters):
                return True
...
for i in range(0, len(list)):
    ...
    while (j < len(list[i])):
        ...
        ## part 1
        if (list[i][j].isdigit() and check_around(list, i, j)):

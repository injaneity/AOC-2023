...
def check_star(x, y):
    for i in range(-1, 2):
        for j in range(-1, 2):
            ...
            stx = star_idx(x + i, len(list))
            sty = star_idx(y + j, len(list[0]))
...
def find_num(x, y):
    ...
    print(list[star[0]][star[1]])
    
    for i in range(-1, 2):
        for j in range(-1 ,2):
            
            
            stx = star_idx(star[0] + i, len(list))
            sty = star_idx(star[1] + j, len(list[0]))
            if ((stx != x or sty > y) and list[stx][sty].isdigit()):
                print("hello" + list[stx][sty])
                ## set char as string
                str2 = list[stx][sty]
                
                ## check behind and add
                count = 1
                while(sty - count >= 0 and list[stx][sty - count].isdigit()):
                    str2 = list[stx][sty - count] + str2
                    count += 1
                    
                ## check in front and add
                count = 1
                while (sty + count < len(list[stx]) and list[stx][sty + count].isdigit()):
                    str2 = str2 + list[stx][sty + count]
                    count += 1
...
while (i < len(list)):
    ...
    while (j < len(list[0])):
        ...
        if (list[i][j].isdigit() and check_star(i, j) and star not in star_list):
            ...
            ## check behind and add
            count = 1
            while(j - count >= 0 and list[i][j - count].isdigit()):
                str1 = list[i][j - count] + str1
                count += 1
                
            ## check in front and add
            count = 1
            while (j + count < len(list[i]) and list[i][j + count].isdigit()):
                str1 = str1 + list[i][j + count]
                count += 1
            
            # jump ahead to skip number
            j += count
            
            
            print("num1 is", str1)
            print("star coordinates: ", str(star[0]), str(star[1]))
